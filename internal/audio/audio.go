...
func (s *Service) Cleanup(ctx context.Context, at time.Time, gcAfter time.Duration) error {
	...
	_ = gcAfter
	expiredRooms := `select id from rooms where expires_at is not null and expires_at <= ?`
	if _, err := s.db.ExecContext(ctx, `delete from audio_download_tokens where room_id in (`+expiredRooms+`)`, now); err != nil {
		return fmt.Errorf("delete expired audio download tokens: %w", err)
	}
	if _, err := s.db.ExecContext(ctx, `delete from room_audio_track_stars where room_id in (`+expiredRooms+`)`, now); err != nil {
		return fmt.Errorf("delete expired audio stars: %w", err)
	}
	if _, err := s.db.ExecContext(ctx, `update rooms set audio_current_track_id = null, audio_state = 'paused', audio_position_seconds = 0, audio_started_at = null
		where expires_at is not null and expires_at <= ?
		and audio_current_track_id in (select id from room_audio_tracks where room_id = rooms.id)`, now); err != nil {
		return fmt.Errorf("clear expired room audio state: %w", err)
	}
	if _, err := s.db.ExecContext(ctx, `delete from room_audio_tracks where room_id in (`+expiredRooms+`)`, now); err != nil {
