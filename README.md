...
- `SLIDETALK_AUDIO_FILES_GC_AFTER`: deprecated compatibility setting; room survival controls audio cleanup, default `7d`
...
Room moderators can upload or replace the PDF, PNG, JPEG, WebP, or GIF slide file attached to their room. Room uploaded files use a room-level survival deadline shown in the Cache panel. Room moderators can prolong survival up to 10 days from now. Site admins who are room members can change survival no earlier than 24 hours from now, or set the room to never expire, even when they are not room moderators. Never-expire rooms are excluded from server cleanup. Rooms use a single mode setting: slides, markdown, or audio. In audio mode, the shared audio player, playlist, finish behavior, and upload controls move into the main stage instead of a right-side audio rail. Room members can see and download audio by default. Optional room settings let non-observer participants upload audio and separately control synced playback for everyone; moderators can also grant upload and synced control access to individual participants from participant cards when the room-wide setting is off. Anyone with synced audio control access can change Finish behavior. Any room member can turn on Local mode (unsynced) to privately play, pause, seek, select tracks, and choose local Finish behavior without changing shared playback; people using manual Local mode show a Local audio badge in the people list. Observers cannot upload or control synced playback. Site admins bypass the audio per-file size limit after a styled confirmation, but all uploads remain subject to the minimum free-space floor. Audio uploads support common browser-safe formats including MP3, M4A, AAC, Ogg/Opus, WAV, FLAC, and WebM audio; the browser skips clearly unsupported selections before upload while the server remains the authoritative validator.

Audio downloads can be converted into room-track download links with a random bearer token in the URL. These links are intended for external download managers and remain valid until the track is removed from the room; the server stores only a hash of the token and does not include browser auth tokens or user IDs in the URL. Each room member can privately star tracks and filter the playlist to starred tracks. Moderators can enable aggregate star counts for the room. Browsers stream uncached audio immediately, fill persistent IndexedDB audio and slide caches in the background, pre-cache the server-announced next audio track, seed those caches directly after successful local uploads, reuse cached blobs for playback/manual downloads/slide viewing after refresh, remember each room member's local mute and volume preferences in localStorage, offer local unsynced playback during manual Local mode or after a sustained realtime disconnect, and resync shared playback when local drift exceeds `SLIDETALK_AUDIO_DRIFT_THRESHOLD`. Shuffle mode uses a deterministic per-room order so clients can cache the actual upcoming track and Previous/Next remain inverse operations within that stable order. Each browser cache garbage-collects entries older than 30 days and trims oldest entries beyond 2 GiB or 500 files. Active rooms include a Cache section in the control rail for local audio/slide usage totals, server-side room survival, prolong controls, and reset controls. Moderators also see a Restore cached audio action when that browser profile has cached audio blobs; it uploads those cached blobs into the currently open room and cannot recover tracks that are no longer present in that browser cache.
...
Active rooms use a room-first workspace: the slide, markdown, or audio stage takes most of the viewport, the top chrome is reduced to a compact timer row with icon-only timer, copy-link, and hand controls, and room controls live in a collapsible side rail. Audio panels keep a compact bottom mini-player with transport, seek, volume, local mode, and Finish controls visible while the searchable track list scrolls above it; search filters by title, uploader, and original filename, and combines with the Starred filter. On mobile, audio track action buttons wrap below track names so moderator controls do not crowd long titles. Participant and observer rail sections show online/total counts and list online members before offline members. The starting page profile, create-room, and join-room controls are hidden while a room is open.

Keyboard shortcuts are ignored while typing in form fields. Browser keyboard media keys control audio playback when available: play/pause toggles the effective local-or-synced audio state, previous/next skip tracks, and rewind/fast-forward seek by 10 seconds. Press `?` in a room to open the shortcuts panel and change local key bindings. Defaults:
